...
public interface SolicitationRepository extends JpaRepository<Solicitation,Integer> {
    ...
    @Query(value = "select s.* from solicitation_product sp " +
                ...
                " where(lower(s.solicitation_name) like concat('%', lower( :name ) , '%') or :name  is null) and " +
                " (lower(p.description) like concat('%', lower( :description ) , '%') or :description is null) and" +
                " (lower(u.full_name) like concat('%',lower( :username ) ) or :username is null) and " +
                " (s.enabled = :enabled or :enabled is null)  and " +
                " (u.pword = :password or :password is null)and " +
                " (lower(p.product_name) like concat('%', lower( :productName ) , '%')  or :productName is null) and " +
